...
fn calculate_grid(paths: &Vec<Vec<(usize, usize)>>) -> HashMap<(usize, usize), Tile> {
    ...
    grid.insert((500, max_row + 2), Tile::Rock);  // placeholder for the rock floor
...
fn simulate(grid: &mut HashMap<(usize, usize), Tile>) -> bool {
    let mut min_col = grid.keys().map(|&(c, _)| c).min().unwrap();
    let mut max_col = grid.keys().map(|&(c, _)| c).max().unwrap();
    // let min_row = grid.keys().map(|&(_, r)| r).min().unwrap();
    let max_row = grid.keys().map(|&(_, r)| r).max().unwrap();
    ...
    loop {
        ...
        if c == max_col { max_col += 1; }
        if r + 1 == max_row {
            for c2 in min_col..=max_col {
                grid.insert((c2, r + 1), Tile::Rock);
            }
        }
        if r >= max_row {
            unreachable!();
