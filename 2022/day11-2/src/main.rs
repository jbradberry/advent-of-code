...
fn main() {
    ...
    // Since we are using modulo arithmetic for each test, we just
    // need to ensure that each item's worry value maintains the same
    // modulo value for each possible monkey.  Taking the product of
    // all of the monkey's tests should do that.
    let modulus = monkeys.iter().fold(1, |acc, m| acc * m.test);
        ...
        for i in 0..monkeys.len() {
            let mut throws = Vec::new();
            {
                ...
                for item in &current_monkey.items {
                    ...
                    let mut new_level = op(a, b).unwrap();
                    new_level = new_level % modulus;
